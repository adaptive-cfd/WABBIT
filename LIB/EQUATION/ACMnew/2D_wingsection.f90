    ...
    do i = 1, nwings
        ...
        do iy = g+1, Bs(2)+g
            ...
            do ix = g+1, Bs(1)+g
                ...
                tmp = step(abs(xp-0.5_rk), 0.5_rk, h, 5*h, params_acm%smoothing_type_int(i_geom))
                tmp = tmp*step(abs(yp), 0.5_rk*wingsections(i)%section_thickness, h, 5*h, params_acm%smoothing_type_int(i_geom))
