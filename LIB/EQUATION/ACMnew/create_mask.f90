...
subroutine create_mask_3D_ACM( time, x0, dx, Bs, g, mask, stage )
    ...
    do i_geom= 1, params_acm%n_geometries
        select case (params_acm%geometries(i_geom))
        ...
        case ('composition')
            if (stage == "init_stage") then
                ...
                if (params_acm%geometry_files(i_geom) /= "") then
                    call init_composition(center=params_acm%x_cntr, scale=params_acm%length, color_set=params_acm%geometry_colors(i_geom), file=params_acm%geometry_files(i_geom), i_composition=i_geom)
                elseif (params_acm%geometry_string /= "") then
                    call init_composition(center=params_acm%x_cntr, scale=params_acm%length, color_set=params_acm%geometry_colors(i_geom), string=params_acm%geometry_string, i_composition=i_geom)
                else
            ...
            elseif (stage == "time-independent-part" .or. stage == "all-parts") then
                ...
                call draw_composition(mask, x0, dx, Bs, g, smoothing_type=params_acm%smoothing_type, smoothing_width=params_acm%smoothing_width, smoothing_safety=params_acm%smoothing_safety, i_composition=i_geom)
            endif
...
subroutine create_mask_2D_ACM( time, x0, dx, Bs, g, mask, stage )
    ...
    do i_geom = 1, params_acm%n_geometries
        ...
        case ('circle', 'cylinder')  ! someone called this cylinder, but it is actually a circle in 2D
            if (stage == "time-independent-part" .or. stage == "all-parts") then
                call draw_circle( mask(:,:,1,:), x0, dx, Bs, g, params_acm%x_cntr(1:2), params_acm%R_cyl, color_set=1_ik, smoothing_type=params_acm%smoothing_type, smoothing_width=params_acm%smoothing_width, smoothing_safety=params_acm%smoothing_safety )
            endif
        ...
        case ('two-circles', 'two-cylinders')  ! actually two circles in 2D
            if (stage == "time-independent-part" .or. stage == "all-parts") then
                ...
                call draw_circle( mask(:,:,1,:), x0, dx, Bs, g, (/0.5884_rk*params_acm%domain_size(1), 0.4116_rk*params_acm%domain_size(2)/), params_acm%R_cyl, color_set=1_ik, smoothing_type=params_acm%smoothing_type, smoothing_width=params_acm%smoothing_width, smoothing_safety=params_acm%smoothing_safety )
                call draw_circle( mask(:,:,1,:), x0, dx, Bs, g, (/0.4116_rk*params_acm%domain_size(1), 0.5884_rk*params_acm%domain_size(2)/), params_acm%R_cyl, color_set=1_ik, smoothing_type=params_acm%smoothing_type, smoothing_width=params_acm%smoothing_width, smoothing_safety=params_acm%smoothing_safety )
            endif
        ...
        case ('cavity')
            if (stage == "time-independent-part" .or. stage == "all-parts") then
                ...
                ! call draw_rectangle( mask, x0, dx, Bs, g, center=(/0.5_rk*params_acm%domain_size(1), 0.0_rk/), half_size=(/1.0e6_rk, params%acm_length/), angle=0.0_rk, color_set=1_ik, smoothing_type=params_acm%smoothing_type, smoothing_width=params_acm%smoothing_width, smoothing_safety=params_acm%smoothing_safety )
                ! call draw_rectangle( mask, x0, dx, Bs, g, center=(/0.0_rk, 0.5_rk*params_acm%domain_size(1)/), half_size=(/params%acm_length, 1.0e6_rk/), angle=0.0_rk, color_set=1_ik, smoothing_type=params_acm%smoothing_type, smoothing_width=params_acm%smoothing_width, smoothing_safety=params_acm%smoothing_safety )
...
subroutine draw_free_cylinder(mask, x0, dx, Bs, g, insect_id )
    ...
    do iy = 1, Bs(2)+2*g
        ...
        do ix = 1, Bs(1)+2*g
            ...
            tmp = step_cosine(r, params_acm%R_cyl, h)
...
subroutine draw_plate_free(mask, x0, dx, Bs, g, insect_id )
    ...
    do iy = 1, Bs(2)+2*g
        ...
        do ix = 1, Bs(1)+2*g
            ...
            tmpx = step_cosine( abs(x), 0.5_rk*params_acm%length, h)
            tmpy = step_cosine( abs(y), 0.5_rk*params_acm%thickness, h)
    ...
    do iz = g+1, Bs(3)+g
        ...
        do iy = g+1, Bs(2)+g
            ...
            do ix = g+1, Bs(1)+g
                ...
                mask(ix,iy,iz,1) = step_cosine(r, params_acm%R_cyl, h)
...
subroutine draw_cylinderz(x0, dx, Bs, g, mask )
    ...
    ! parameter for smoothing function (width)
    h = params_acm%C_smooth*minval(dx)
...
subroutine draw_cavity(mask, x0, dx, Bs, g )
    ...
    do iy = 1, Bs(2)+2*g
        ...
        do ix = 1, Bs(1)+2*g
            ...
            mask(ix,iy,1) = step_cosine( tmp, length, c_smooth*params_acm%dx_min)
