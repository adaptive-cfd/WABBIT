...
subroutine RHS_ACM( time, u, g, x0, dx, rhs, mask, stage, n_domain )
                    ...
                    do iy = g+1, Bs(2)+g
                        ...
                        do ix = g+1, Bs(1)+g
                            ...
                            if (any(color == (/insects(i_insect)%color_body, insects(i_insect)%color_l, insects(i_insect)%color_r, insects(i_insect)%color_l2, insects(i_insect)%color_r2/))) then
                                ...
                                f_block(1:params_acm%dim) = f_block(1:params_acm%dim) - penal(1:params_acm%dim)
    ...
    case ("local_stage")
        ...
        if (params_acm%use_passive_scalar) then
            call RHS_scalar(g, Bs, dx, x0, u, params_acm%discretization, time, rhs, mask, n_domain)
        endif
        ...
        if (params_acm%skew_symmetry) then
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    ...
                    penalx = -mask(ix,iy,1) * C_eta_apply_inv(color) * (phi(ix,iy,1) -mask(ix,iy,2))
                    penaly = -mask(ix,iy,1) * C_eta_apply_inv(color) * (phi(ix,iy,2) -mask(ix,iy,3))
            ...
            do iz = g+1, Bs(3)+g
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ...
                        chi = mask(ix,iy,iz,1) * C_eta_apply_inv( int(mask(ix,iy,iz,5), kind=2) )
...
subroutine RHS_scalar(g, Bs, dx, x0, phi, order_discretization, time, rhs, mask, n_domain)
    ...
    do iscalar = 1, params_acm%N_scalars
        ...
        case ("mask_color_emission")
            do iz = iz1, iz2
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        if ( abs(mask(ix,iy,iz,5) - params_acm%widthsource(iscalar)) <= 1.0e-8 ) then
    ...
    if (params_acm%use_active_buoyancy_scalar) then
        ...
        do iscalar = 1, params_acm%N_scalars
            ...
            do iz = iz1, iz2
                ...
                if (dim == 3) then
                    ...
                    if (params_acm%symmetric_buoyancy .and. z <= params_acm%domain_size(3)/2.0_rk) active_factor = 1.0_rk
                else
                    z = 0.0_rk
                endif
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        rhs(ix,iy,iz,3) = rhs(ix,iy,iz,3) + active_factor * params_acm%rayleigh_numbers(iscalar) * params_acm%schmidt_numbers(iscalar) * phi(ix,iy,iz,j)
                    end do
                end do
            end do
        enddo
