    ...
    case ("post_stage")
        ...
        if (is_insect) then
            do i_insect = 1, n_insects
                ...
                Insects(i_insect)%PartIntegrals( 1 )%Torque = params_acm%moment_color(1:3, Insects(i_insect)%color_body )
                Insects(i_insect)%PartIntegrals( 2 )%Torque = params_acm%moment_color(1:3, Insects(i_insect)%color_l )
                Insects(i_insect)%PartIntegrals( 3 )%Torque = params_acm%moment_color(1:3, Insects(i_insect)%color_r )
                if (Insects(i_insect)%second_wing_pair) then
                    Insects(i_insect)%PartIntegrals( 4 )%Torque = params_acm%moment_color(1:3, Insects(i_insect)%color_l2 )
                    Insects(i_insect)%PartIntegrals( 5 )%Torque = params_acm%moment_color(1:3, Insects(i_insect)%color_r2 )
                endif
