...
subroutine update_grid_qtys_ACM( time, field, g, x0, dx, stage )
    ...
    case("main_stage")
        ...
        if ( params_acm%geometry == "Insect" .and. Insect%body_moves == "no" .and. params_acm%dim==3 ) then
            ...
            call Draw_Insect( time, Insect, x0, dx, field(g+1:Bs+g,g+1:Bs+g,g+1:Bs+g,IDX_MASK), &
            mask_color, field(g+1:Bs+g,g+1:Bs+g,g+1:Bs+g,IDX_USX:IDX_USZ), with_body = .true., &
