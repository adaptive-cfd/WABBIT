module module_geometry
    ...
    ! functions
    PUBLIC :: signed_distance_triangle_2D, signed_distance_circle_2D, signed_distance_rectangle_2D, signed_distance_sphere_3D, signed_distance_cylinder_3D, signed_distance_capsule_3D, draw_sphere, draw_cylinder, draw_capsule, draw_circle, draw_rectangle, draw_triangle, init_composition, draw_composition
    ...
    type(composition) :: geom_array(20)  ! we can have up to 20 compositions, which should be enough for most cases. We can make this allocatable if we want to be fancy, but I don't see the need for now.
...
#include "geometry_compositions.f90"
!---------------------------------------
...
subroutine draw_sphere(mask, x0, dx, Bs, g, center, radius, color_set, smoothing_type, smoothing_width, smoothing_safety, bounding_box )
    use module_globals
    ...
    character(len=cshort), intent(in) :: smoothing_type     !< which mask do we use?
    ...
    integer(kind=ik) :: iz, ix, iy, Nsafety, bound_min(1:3), bound_max(1:3)

    abstract interface
        function step_function(dist, center, width, safety) result(val)
            import rk
            real(rk), intent(in) :: dist, center, width, safety
            real(rk) :: val
        end function
    end interface
    procedure(step_function), pointer :: step
    
    ! select the correct smoothing function. Here 4 means that the function has 4 input parameters
    step => step_cosine4  ! to avoid 'may be unitialized' warnings
    ...
    case ('dis', 'disc', 'discontinuous')
        step => step_disc4
    case default
        call abort(260603, "Not so smooth! Smoothing_type not recognized: " // smoothing_type)
    end select
...
!! Outputs:
!! - `mask` updated with the geometry, ID-1 is mask array, ID-5 is color
subroutine draw_cylinder(mask, x0, dx, Bs, g, endpoint_1, endpoint_2, radius, color_set, smoothing_type, smoothing_width, smoothing_safety, bounding_box )
    use module_globals
...
!---------------------------------------
!> Draw a capsule / cylinder with spherical end points using the exact SDF
!!
!! This is a pointwise implementation that loops over all grid points in the bounding box of the cylinder and applies the mask function based on the signed distance to the cylinder.
!!
!! Inputs:
...
!! Outputs:
!! - `mask` updated with the geometry, ID-1 is mask array, ID-5 is color
subroutine draw_capsule(mask, x0, dx, Bs, g, endpoint_1, endpoint_2, radius, color_set, smoothing_type, smoothing_width, smoothing_safety, bounding_box )
    use module_globals
    ...
    !> grid information
    integer(kind=ik), intent(in) :: Bs(1:3), g
    real(kind=rk), dimension(3), intent(in) :: endpoint_1, endpoint_2  !< capsule endpoints
    real(kind=rk), intent(in) :: radius                !< capsule radius
    integer(kind=ik), intent(in) :: color_set               !< which color to set the mask
    character(len=cshort), intent(in) :: smoothing_type     !< which mask do we use?
    real(kind=rk), intent(in) :: smoothing_width       !< width of smoothing region
    real(kind=rk), optional, intent(in) :: smoothing_safety      !< safety margin for smoothing
    ...
    ! soem auxiliary variables for the capsule, speeds up point-wise computations
    ba = endpoint_2 - endpoint_1
    baba = dot_product(ba, ba)

    ! bounding box check for capsule
    ! for capsule, we can go at most radius away from the endpoints.
    if (present(bounding_box)) then
        ! use pre-computed bounding box for sphere (can be faster if many spheres are to be drawn)
        bound_min = int((bounding_box(1:3)-x0)/dx) - Nsafety + g+1
        bound_max = int(ceiling((bounding_box(4:6)-x0)/dx)) + Nsafety + g+1
    ...
    do iz = max(bound_min(3),lbounds(3)), min(bound_max(3),ubounds(3))
        ...
        do iy = max(bound_min(2),lbounds(2)), min(bound_max(2),ubounds(2))
            ...
            do ix = max(bound_min(1),lbounds(1)), min(bound_max(1),ubounds(1))
                xyz(1) = dble(ix-(g+1)) * dx(1) + x0(1)
                ! distance from center of capsule
                dist = signed_distance_capsule_3D( xyz, endpoint_1, endpoint_2, radius, ba_in=ba, baba_in=baba )
                ! apply mask function
                tmp = step(dist, 0.0_rk, smoothing_width, safety)
                if (tmp >= mask(ix,iy,iz,1)) then
                    mask(ix,iy,iz,1) = tmp    ! mask function
...
!! Outputs:
!! - `mask` updated with the geometry, ID-1 is mask array, ID-5 is color
subroutine draw_circle(mask, x0, dx, Bs, g, center, radius, color_set, smoothing_type, smoothing_width, smoothing_safety, bounding_box )
    use module_globals
    ...
    integer(kind=ik) :: ix, iy, Nsafety, bound_min(1:2), bound_max(1:2)
    ...
    do iy = max(bound_min(2),lbounds(2)), min(bound_max(2),ubounds(2))
        ...
        do ix = max(bound_min(1),lbounds(1)), min(bound_max(1),ubounds(1))
            ...
            if (tmp >= mask(ix,iy,1)) then
                mask(ix,iy,1) = tmp    ! mask function
                mask(ix,iy,5) = real(color_set, kind=rk) ! color
            endif
    end do; end do
...
!! Outputs:
!! - `mask` updated with the geometry, ID-1 is mask array, ID-5 is color
subroutine draw_rectangle(mask, x0, dx, Bs, g, center, half_size, angle, color_set, smoothing_type, smoothing_width, smoothing_safety, bounding_box )
    use module_globals
...
!! Outputs:
!! - `mask` updated with the geometry, ID-1 is mask array, ID-5 is color
subroutine draw_triangle(mask, x0, dx, Bs, g, vertex1, vertex2, vertex3, color_set, smoothing_type, smoothing_width, smoothing_safety, bounding_box )
    use module_globals
