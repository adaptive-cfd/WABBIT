...
subroutine draw_wing(xx0, ddx, mask, mask_color, us, Insect, color_wing, M_body,&
  ...
  select case(Insect%WingShape(idw))
  case ("pointcloud")
  ...
  case default
    ...
    wingshape_str = Insect%WingShape(idw)
              ...
              if ( x_wing(1) >= Insect%wing_bounding_box(1,idw)-s .and. x_wing(1) <= Insect%wing_bounding_box(2,idw)+s) then
                  if ( x_wing(2) >= Insect%wing_bounding_box(3,idw)-s .and. x_wing(2) <= Insect%wing_bounding_box(4,idw)+s) then
                      if ( x_wing(3) >= Insect%wing_bounding_box(5,idw)-s .and. x_wing(3) <= Insect%wing_bounding_box(6,idw)+s) then
                          ...
                          theta = atan2( x_wing(2)-Insect%yc(idw), x_wing(1)-Insect%xc(idw) )
                          theta = ( theta + pi ) / (2.d0*pi)

                          !-- construct R by evaluating the fourier series
                          R0 = Radius_Fourier(theta,Insect,idw)

                          !-- get smooth (radial) step function
                          R = dsqrt ( (x_wing(1)-Insect%xc(idw))**2 + (x_wing(2)-Insect%yc(idw))**2 )
...
!                  if ( x_wing(2) >= Insect%wing_bounding_box(3,idw)-s .and. x_wing(2) <= Insect%wing_bounding_box(4,idw)+s) then
                  if ( x_wing(2) > 0.0d0 .and. x_wing(2) < rblade ) then
                      if ( x_wing(3) >= Insect%wing_bounding_box(5,idw)-s .and. x_wing(3) <= Insect%wing_bounding_box(6,idw)+s) then
                          ...
                          xle = Radius_Fourier(theta,Insect,idw)
...
!-------------------------------------------------------------------------------
real(kind=rk) function Radius_Fourier( theta, Insect, idw )
  implicit none
  ...
  if (.not.Insect%wings_radius_table_ready(idw)) then
    !---------------------------------------------------------------------------
    ! fill radius table
    !---------------------------------------------------------------------------
    Insect%R0_table(:,idw) = 0.d0
    ! loop over all thetas and compute the radius for all of them, store it
    ! in the table Insect%R0_table
    do j = 1, n_radius
      R0 = Insect%a0_wings(idw) / 2.d0
      theta2 = dble(j-1) * dphi
      ! evaluate Fourier series
      do i = 1, Insect%nfft_wings(idw)
        R0 = R0 + Insect%ai_wings(i,idw)*dcos(2.d0*pi*dble(i)*theta2) &
                + Insect%bi_wings(i,idw)*dsin(2.d0*pi*dble(i)*theta2)
      enddo
      Insect%R0_table(j,idw)=R0
    enddo

    ! skip setup on next call
    Insect%wings_radius_table_ready(idw) = .true.
  endif

  ! linear interpolation, if already stored the radius
  j = floor( theta / dphi ) + 1
  Radius_Fourier = Insect%R0_table(j,idw) + ((theta-dble(j-1)*dphi) / dphi) &
                 * (Insect%R0_table(j+1,idw)-Insect%R0_table(j,idw))
end function
...
subroutine compute_wing_surface( Insect, idw, area )
  implicit none
  type(diptera),intent(inout) :: Insect
  ...
  if ( Insect%nfft_wings(idw) /= 0 ) then
...
! series.
!-------------------------------------------------------------------------------
subroutine Setup_Wing_Fourier_coefficients(Insect,idw)
  implicit none
  ...
  if (Insect%wingsetup_done(idw)) then
    ! the second call is just a return statement
    return
  endif

  Insect%a0_wings(idw) = 0.d0
  Insect%ai_wings(:,idw) = 0.d0
  Insect%bi_wings(:,idw) = 0.d0
  ...
  case ('pieris-brassicae1')
    ...
    Insect%nfft_wings(idw) = 30
    Insect%a0_wings(idw) = 0.8262378
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = & 
    ...
    Insect%bi_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    Insect%yc(idw) = 0.3184928
    Insect%xc(idw) = -0.2459908

  case ('drosophila')
    !********************************************
    ! Drosophila wing from Jan Gruber's png file
    !********************************************
    Insect%nfft_wings(idw) = 40
    Insect%a0_wings(idw) = 0.5140278
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    yroot =-0.0157
    ! center of circle
    Insect%xc(idw) =-0.1206 + xroot
    Insect%yc(idw) = 0.3619 + yroot
  case ('drosophila_mutated')
    !********************************************
    ! mutated Drosophila wing from Jan Gruber's png file
    !********************************************
    Insect%nfft_wings(idw) = 70
    Insect%a0_wings(idw) = 0.4812548
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
  ...
  case ('drosophila_sandberg')
    ...
    !********************************************
    Insect%nfft_wings(idw) = 24
    Insect%a0_wings(idw) = 0.4995578
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    /)
    Insect%xc(idw) =-0.0235498
    Insect%yc(idw) = 0.1531398
  case ('drosophila_maeda')
    !********************************************
    !  Drosophila wing from Maeda and Liu, similar to Liu and Aono, BB2009
    !********************************************
    Insect%nfft_wings(idw) = 25
    !Insect%a0_wings(idw) = 0.591294836514357
    ...
    Insect%a0_wings(idw) = 0.585432698694358
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    !Insect%xc(idw) = 0.0 ! original mesh
    Insect%xc(idw) = 0.0473 ! shifted towards t.e. to 1/4 of the root chord ("+" sign here)
    !Insect%xc(idw) = -0.0728 ! shifted towards l.e., to 0.2cmean from the l.e. (Liu and Aono BB 2009)
    !Insect%yc(idw) = 0.7
    !Insect%yc(idw) = 0.712 ! measured using kinematics snapshots
    Insect%yc(idw) = 0.702 ! According to Maeda's email, Jun 21, 2014
  case ('drosophila_sun')
    !********************************************
    !  Drosophila virilis wing from Chen and Sun, Acta Mech Sin 2014
    !********************************************
    Insect%nfft_wings(idw) = 25
    Insect%a0_wings(idw) = 0.5427789795180327
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    Insect%xc(idw) = 0.0
    Insect%yc(idw) = 0.399446382250523
  case ('bumblebee')
    !********************************************
    !  Bumblebee
    ...
    !********************************************
    Insect%nfft_wings(idw) = 25
    Insect%a0_wings(idw) = 0.594557593733011d0
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    Insect%xc(idw) = -0.1d0
    Insect%yc(idw) = 0.501549263807117d0
  ...
  case ('b_ignitus')
    ...
    !********************************************
    Insect%nfft_wings(idw) = 25
    Insect%a0_wings(idw) = 0.536472532931637d0
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    Insect%xc(idw) = -0.13d0
    Insect%yc(idw) = 0.434820393790595d0
  ...
  case ('paratuposa_flatwing')
    ...
    !********************************************
    Insect%nfft_wings(idw) = 15
    Insect%a0_wings(idw) = 0.694542662069373d0
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
  ...
  case ('paratuposa_flatelytra')
    ...
    !  Digitized from 3D model, Moscow University entomology lab
    !********************************************
    Insect%nfft_wings(idw) = 21
    Insect%a0_wings(idw) = 0.565146320110115d0 * 0.62d0
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    * 0.62d0
    Insect%xc(idw) = -0.02d0 * 0.62d0
    Insect%yc(idw) = 0.65d0 * 0.62d0
  ...
  case ('flapper_sane')
    ...
    !********************************************
    Insect%nfft_wings(idw) = 25
    Insect%a0_wings(idw) = 0.5379588906565078
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
  ...
  case ('flapper_dickinsonII')
    ...
    !********************************************
    Insect%nfft_wings(idw) = 20
    Insect%a0_wings(idw) = 0.6442788
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    -0.0005798,0.0001228/)
    Insect%yc(idw) = 0.5282438
    Insect%xc(idw) = -0.1184548
  ...
  case ('robofly_dickinson')
    ...
    !********************************************
    Insect%nfft_wings(idw) = 28
    Insect%a0_wings(idw) = 0.5313628
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    Insect%yc(idw) = 0.4645238
    Insect%xc(idw) = -0.0716018
  ...
  case ('hawkmoth1')
    ...
    Insect%yc(idw) = 0.4171918
    Insect%xc(idw) = -0.0395258
  case ('hawkmoth2')
    ...
    ! for this wing shape.
    Insect%nfft_wings(idw) = 18
    Insect%a0_wings(idw) = 0.6617728
    Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = &
    ...
    Insect%yc(idw) = 0.3946798
    Insect%xc(idw) = -0.2157968
  ...
  case default
    ...
    if (index(wingshape_str,"from_file::") /= 0) then
      ...
      !-------------------------------------------------------------------------
      call Setup_Wing_from_inifile(Insect, idw, trim(adjustl(wingshape_str( 12:len_trim(wingshape_str) ))))
  ...
  Insect%wingsetup_done(idw) = .true.
  ...
  call set_wing_bounding_box_fourier( Insect, idw )

  ! this is the old defaut value:
  if (maxval(Insect%corrugation_array_bbox) == 0.0_rk) then
      Insect%corrugation_array_bbox(1:4) = Insect%wing_bounding_box(1:4,idw)
  endif

  if (root) then
    write(*,'(30("-"))')
    write(*,'("Insect module: Setup_Wing_Fourier_coefficients")')
    write(*,'("Wing shape is ",A)') trim(adjustl(Insect%WingShape(idw)))
    write(*,'("nfft_wings=",i3)') Insect%nfft_wings(idw)
    write(*,'(30("-"))')
  endif
...
!-------------------------------------------------------------------------------
subroutine Setup_Wing_from_inifile( Insect, idw, fname )
  implicit none
  type(diptera),intent(inout) :: Insect
  ...
  !-----------------------------------------------------------------------------
  call read_param_mpi( ifile, "Wing", "a0_wings", Insect%a0_wings(idw), 0.d0)
  ...
  Insect%ai_wings(1:Insect%nfft_wings(idw),idw) = tmparray(1,:)
  deallocate(tmparray)
  ...
  Insect%nfft_wings = size(tmparray,2)
  Insect%bi_wings(1:Insect%nfft_wings(idw),idw) = tmparray(1,:)
  deallocate(tmparray)

  ! wing mid-point (of course in wing system..)
  call read_param_mpi(ifile,"Wing","x0w",Insect%xc(idw), 0.d0)
  call read_param_mpi(ifile,"Wing","y0w",Insect%yc(idw), 0.d0)
...
!-------------------------------------------------------------------------------
subroutine set_wing_bounding_box_fourier( Insect, idw )
  implicit none
  type(diptera),intent(inout) :: Insect
  ...
  do while ( theta < 1.d0 )
    ...
    R = Radius_Fourier( theta, Insect, idw )

    theta_prime = 2.d0*pi*theta - pi
    x = Insect%xc(idw) + R * cos( theta_prime )
    y = Insect%yc(idw) + R * sin( theta_prime )
  ...
  Insect%wing_bounding_box(1:4,idw) = (/xmin, xmax, ymin, ymax/)

  ! the bounding box in z-direction depends on the wing thicnkess (constant or not)
  ! and the corrugation
  if ( Insect%wing_thickness_distribution == "constant" ) then
    if ( Insect%corrugated ) then
      Insect%wing_bounding_box(5,idw) = minval(corrugation_profile) - Insect%WingThickness / 2.0_pr
      Insect%wing_bounding_box(6,idw) = maxval(corrugation_profile) + Insect%WingThickness / 2.0_pr
    else
      ! constant thickness, no corrugation is the classical flat case:
      Insect%wing_bounding_box(5,idw) = -Insect%WingThickness / 2.0_pr
      Insect%wing_bounding_box(6,idw) = +Insect%WingThickness / 2.0_pr
    endif
  else
    if ( Insect%corrugated ) then
      ! minimum of lower surface
      Insect%wing_bounding_box(5,idw) = minval(corrugation_profile-wing_thickness_profile/2.0_pr)
      ! maximum of upper surface
      Insect%wing_bounding_box(6,idw) = maxval(corrugation_profile+wing_thickness_profile/2.0_pr)
    else
      ! bounding box is +- largest thickness  simply
      Insect%wing_bounding_box(5,idw) = -maxval(wing_thickness_profile / 2.0_pr)
      Insect%wing_bounding_box(6,idw) =  maxval(wing_thickness_profile / 2.0_pr)
    endif
  end if
  ...
  if (root) then
    ...
    write(*,'("xwmin=",es15.8," xwmax=",es15.8)') Insect%wing_bounding_box(1:2,idw)
    write(*,'("ywmin=",es15.8," ywmax=",es15.8)') Insect%wing_bounding_box(3:4,idw)
    write(*,'("zwmin=",es15.8," zwmax=",es15.8)') Insect%wing_bounding_box(5:6,idw)
  endif
