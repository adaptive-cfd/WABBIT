...
subroutine pipe_flow_penalization2D(Bs, g, x0, dx, mask, phi_ref)
      ...
      do ir = g+1, Bs + g
        do iz = g+1, Bs + g
          ...
          z  = dble(iz-(g+1)) * dx(1) + x0(1)
          if (tmp_mask(iz,ir) > 0.0_rk) then
          ...
          !  mask(iz,ir,UyF ) = C_sp_inv*tmp_mask(iz,ir)
            mask(iz,ir,pF  ) = C_sp_inv*tmp_mask(iz,ir)
          ...
          !  phi_ref(iz,ir,UyF) = 0.0_rk
            if (z > params_ns%domain_size(1)*0.5_rk) then
              phi_ref(iz,ir,pF) = p_out
            else
              phi_ref(iz,ir,pF) = p_in
            end if
          end if
        end do
      end do
