subroutine draw_2d_wingsections(time, mask, x0, dx, Bs, g )
    ...
    do i = 1, nwings
        ...
        dx_min = maxval( (2.0_rk**(-params_nspp%Jmax))*params_nspp%domain_size(1:2)/real(Bs(1:2)-1,kind=rk) )
        h = params_nspp%C_smooth * dx_min
        ...
        do iy = g+1, Bs(2)+g
            ...
            do ix = g+1, Bs(1)+g
                ...
                tmp = step_cosine(abs(xp-0.5_rk), 0.5_rk, h)
                tmp = tmp*step_cosine(abs(yp), 0.5_rk*wingsections(i)%section_thickness, h)
...
subroutine init_wingsection_from_file(file, theSection, time)
    ...
    call read_param_mpi(ifile, "Wingsection", "a0_x0", theSection%a0_x0, 2.0_rk*0.5_rk*params_nspp%domain_size(1))
    ...
    call read_param_mpi(ifile, "Wingsection", "a0_y0", theSection%a0_y0, 2.0_rk*0.5_rk*params_nspp%domain_size(2))
