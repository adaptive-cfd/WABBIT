...
subroutine BOUNDCOND_NSPP( time, u, g, x0, dx, n_domain, spaghetti_form, edges_only)
    implicit none
    ...
    if (.not. params_nspp%initialized) write(*,'(A)') "WARNING: BOUNDCOND_NSPP called but NSPP not initialized"

    ! we do not sync only for state vector, if this is the case, then everything is treated as symmetric
    disable_antisymmetric = size(u,4) /= dim + 1 + params_nspp%N_scalars
    ...
    if (n_domain(1) == -1 .and. params_nspp%symmetry_BC(1)) then
        do is = 1,size(u,4)
    ...
    if (n_domain(2) == -1 .and. params_nspp%symmetry_BC(2)) then
        do is = 1,size(u,4)
    ...
    if (dim == 3) then
        if (n_domain(3) == -1 .and. params_nspp%symmetry_BC(3)) then
            do is = 1,size(u,4)
