...
subroutine RHS_NSPP( time, u, g, x0, dx, rhs, mask, stage, n_domain, discretization_overwrite )
                    ...
                    do iy = g+1, Bs(2)+g
                        ...
                        do ix = g+1, Bs(1)+g
                            ...
                            if (any(color == (/insects(i_insect)%color_body, insects(i_insect)%color_l, insects(i_insect)%color_r, insects(i_insect)%color_l2, insects(i_insect)%color_r2/))) then
                                ! penalization term
                                penal = -mask(ix,iy,iz,1) * (u(ix,iy,iz,1:3) - mask(ix,iy,iz,2:4)) * C_eta_apply_inv(color)
    ...
    case ("local_stage")
        ...
        ! --------------------------------------------------------------------------
        ! passive scalars (combined 2D/3D implementation, phi/rhs/mask keep full rank)
        ! --------------------------------------------------------------------------
        if (params_nspp%use_passive_scalar) then
            call RHS_scalar(g, Bs, dx, x0, u, discretization, time, rhs, mask, n_domain)
        endif
...
subroutine RHS_NSPP_Velocity(g, Bs, dx, x0, phi, order_discretization, time, rhs, mask, n_domain)
    ...
    real(kind=rk) :: dx_inv, dy_inv, dz_inv, dx2_inv, dy2_inv, dz2_inv, C_eta_apply_inv(0:ncolors), C_eta_sponge_inv
    ...
    if (dim == 2) then
        ...
        do iy = g+1, Bs(2)+g
            do ix = g+1, Bs(1)+g
                ...
                penalx = -mask(ix,iy,1,1) * C_eta_apply_inv( color ) * (ux - mask(ix,iy,1,2))
                penaly = -mask(ix,iy,1,1) * C_eta_apply_inv( color ) * (uy - mask(ix,iy,1,3))
        ...
        do iz = g+1, Bs(3)+g
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    ...
                    ! Penalization: -\chi/C_eta * (U - U_target)
                    color = int(mask(ix,iy,iz,5), kind=2)
                    penalx = -mask(ix,iy,iz,1) * C_eta_apply_inv(color) * (ux - mask(ix,iy,iz,2))
                    penaly = -mask(ix,iy,iz,1) * C_eta_apply_inv(color) * (uy - mask(ix,iy,iz,3))
                    penalz = -mask(ix,iy,iz,1) * C_eta_apply_inv(color) * (uz - mask(ix,iy,iz,4))
    ...
    if (params_nspp%use_sponge) then
        ...
        if (dim == 2 .and. (any(params_nspp%geometries(:) == "lamballais") .or. any(params_nspp%geometries(:) == "lamballais-local"))) then
            C_eta_sponge_inv = 1.0_rk / params_nspp%C_eta_ring
            ...
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    ...
                    rhs(ix,iy,1,1) = rhs(ix,iy,1,1) - mask(ix,iy,1,6)*(phi(ix,iy,1,1) - mask(ix,iy,1,2)) * C_eta_sponge_inv
                    rhs(ix,iy,1,2) = rhs(ix,iy,1,2) - mask(ix,iy,1,6)*(phi(ix,iy,1,2) - mask(ix,iy,1,3)) * C_eta_sponge_inv
                    ! Note: Pressure component (if present) would be handled separately in pressure equation
                enddo
            enddo
        else
            C_eta_sponge_inv = 1.0_rk / params_nspp%C_sponge
            ! Standard sponge treatment            
            if (dim == 2) then
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ! Sponge mask is stored in component 6 of mask array
                        spo = mask(ix,iy,1,6) * C_eta_sponge_inv
                ...
                do iz = g+1, Bs(3)+g
                    do iy = g+1, Bs(2)+g
                        do ix = g+1, Bs(1)+g
                            ! Sponge mask is stored in component 6 of mask array
                            spo = mask(ix,iy,iz,6) * C_eta_sponge_inv
...
subroutine RHS_scalar(g, Bs, dx, x0, phi, order_discretization, time, rhs, mask, n_domain)
    use module_operators
    implicit none
    ...
    integer(kind=ik) :: ix, iy, iz, iz1, iz2, iscalar, j, dim
    ...
    real(kind=rk) :: phi_dx, phi_dy, phi_dz, phi_dxdx, phi_dydy, phi_dzdz, active_factor
    ...
    do iscalar = 1, params_nspp%N_scalars
                ...
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ...
                        if ( x <= params_nspp%widthsource(iscalar) ) then
                            ! INFLOW
                            source(ix,iy,iz) = (1.0_rk - phi(ix,iy,iz,j)) * C_eta_apply_inv( int(mask(ix,iy,iz,5), kind=2) ) ! for the source term, we use the usual dirichlet C_eta
                        endif
                    end do
                end do
            end do

        case ("in+outflow-x", "in+outflow")
            do iz = iz1, iz2
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        x = x0(1) + dble(ix-(g+1))*dx(1)
                        ...
                        elseif ( x >= params_nspp%domain_size(1)-params_nspp%widthsource(iscalar) ) then
                            ! OUTFLOW
                            source(ix,iy,iz) = (0.0_rk - phi(ix,iy,iz,j)) * C_eta_apply_inv( int(mask(ix,iy,iz,5), kind=2) ) ! for the source term, we use the usual dirichlet C_eta
                        endif
                    end do
                end do
            end do
        ...
        case ("mask_color_emission")
            do iz = iz1, iz2
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        if ( abs(mask(ix,iy,iz,5) - params_nspp%widthsource(iscalar)) <= 1.0e-8 ) then
                            source(ix,iy,iz) = -mask(ix,iy,iz,5)*(phi(ix,iy,iz,j)-1.d0) * C_eta_apply_inv( int(mask(ix,iy,iz,5), kind=2) )
                        endif
                    end do
                end do
            end do
            ...
            do iz = iz1, iz2
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ...
                        ! assemble everything (x/y part)
                        rhs(ix,iy,iz,j) = wx*gx + wy*gy & ! penalized convection term
                        ...
                        if (dim == 3) then
                            ...
                            rhs(ix,iy,iz,j) = rhs(ix,iy,iz,j) + wz*gz + D_dz*gz + D*gzz
                        endif

                        rhs(ix,iy,iz,j) = rhs(ix,iy,iz,j) + source(ix,iy,iz)
                    end do
                end do
            end do
            ...
            do iz = iz1, iz2
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ...
                        ! easy RHS for dirichlet BC (x/y part)
                        rhs(ix,iy,iz,j) = -ux*phi_dx -uy*phi_dy &
                        ...
                        if (dim == 3) then
                            ...
                            rhs(ix,iy,iz,j) = rhs(ix,iy,iz,j) - uz*phi_dz + kappa*phi_dzdz
                        endif

                        rhs(ix,iy,iz,j) = rhs(ix,iy,iz,j) &
                        - chi*(phi(ix,iy,iz,j) - 0.0_rk) * C_eta_apply_inv( int(mask(ix,iy,iz,5), kind=2) ) & ! Dirichlet penalization for obstacle mask (instead of Neumann penalization)
    ...
    ! active scalar forcing (like buoyancy) is added here, after the rhs has been computed for all scalars
    if (params_nspp%use_active_buoyancy_scalar) then
        ! assume for now that all scalars are active scalars, and that the forcing is a linear combination of all scalars
        ! also assume, that forcing is only applied to the z-momentum equation
        do iscalar = 1, params_nspp%N_scalars
            j = iscalar + (params_nspp%dim + 1)
            do iz = iz1, iz2
                active_factor = -1.0_rk
                if (dim == 3) then
                    z = x0(3) + dble(iz-g-1)*dx(3)
                    ! for symmetric condition, gravity points in different direction if in lower or upper half of the domain
                    if (params_nspp%symmetric_buoyancy .and. z <= params_nspp%domain_size(3)/2.0_rk) active_factor = 1.0_rk
                else
                    z = 0.0_rk
                endif
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        rhs(ix,iy,iz,3) = rhs(ix,iy,iz,3) + active_factor * params_nspp%rayleigh_numbers(iscalar) * params_nspp%schmidt_numbers(iscalar) * phi(ix,iy,iz,j)
                    end do
                end do
            end do
        enddo
