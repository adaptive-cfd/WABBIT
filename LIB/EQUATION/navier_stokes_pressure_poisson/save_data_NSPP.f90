    ...
    do k = 1, n_names
        ...
        if (name(1:6) == "scalar") then
            ...
            if (.not. (name(7:7) >= '0' .and. name(7:7) <= '9')) then
                call abort(250920, "ERROR: PREPARE_SAVE_DATA_NSPP: field_name '"//trim(name)//"' has invalid format. Expected 'scalarX' where X is a digit (1-9), but found '"//name(7:7)//"' at position 7.")
            end if
