...
subroutine sponge_2D(sponge, x0, dx, Bs, g)
    ...
    if (.not. params_nspp%initialized) write(*,*) "WARNING: sponge_2D called but NSPP not initialized"

    if (params_nspp%sponge_type == "rect") then
        ...
        do iy = g+1, Bs(2)+g
            ...
            do ix = g+1, Bs(1)+g
                ...
                sponge(ix,iy) = step_cosine( tmp, 0.5_rk*params_nspp%L_sponge, 0.5_rk*params_nspp%L_sponge)
            end do
        end do

    elseif (params_nspp%sponge_type == "p-norm") then
        ...
        if ( maxval(abs(params_nspp%domain_size(1:2)-params_nspp%domain_size(1))) > 1.0e-10_rk) then
            call abort(1610184,"ERROR: for the p-norm sponge, the domain has to be same size in all directions.")
        endif

        p = params_nspp%p_sponge
        offset = 0.5_rk * params_nspp%domain_size(1)
        ...
        do iy = g+1, Bs(2)+g
            ...
            do ix = g+1, Bs(1)+g
                ...
                sponge(ix,iy) = step_cosine( tmp, 0.5_rk*params_nspp%L_sponge, &
                0.5_rk*params_nspp%L_sponge)
            end do
        end do
    else
...
subroutine sponge_3D(sponge, x0, dx, Bs, g)
    ...
    if (.not. params_nspp%initialized) write(*,*) "WARNING: sponge_3D called but NSPP not initialized"
    ...
    if (params_nspp%sponge_type == "rect") then
        ...
        tmp = min(minval(x0), minval(params_nspp%domain_size - (x0 + dx*dble(Bs))))
        if (tmp > params_nspp%L_sponge) then
            sponge = 0.0_rk
        else
            ! rectangular sponge with 45deg edges
            do iz = g+1, Bs(3)+g
                ...
                do iy = g+1, Bs(2)+g
                    ...
                    do ix = g+1, Bs(1)+g
                        ...
                        ! distance to borders of domain
                        tmp = minval( (/x,y,z,-(x-params_nspp%domain_size(1)),&
                            -(y-params_nspp%domain_size(2)),-(z-params_nspp%domain_size(3))/) )

                        sponge(ix,iy,iz) = step_cosine( tmp, 0.5_rk*params_nspp%L_sponge, 0.5_rk*params_nspp%L_sponge)
                    end do
                end do
            end do
        endif
    ...
    elseif (params_nspp%sponge_type == "rect-symmetry-y") then
        ! rectangular sponge with 45deg edges
        do iz = g+1, Bs(3)+g
            z = dble(iz-(g+1)) * dx(3) + x0(3)
            do iy = g+1, Bs(2)+g
                ...
                do ix = g+1, Bs(1)+g
                    ...
                    ! distance to borders of domain
                    tmp = minval( (/x,z,-(x-params_nspp%domain_size(1)),-(y-params_nspp%domain_size(2)),-(z-params_nspp%domain_size(3))/) )
    ...
    elseif (params_nspp%sponge_type == "rect-symmetry-yz") then
        ! rectangular sponge with 45deg edges
        do iz = g+1, Bs(3)+g
            z = dble(iz-(g+1)) * dx(3) + x0(3)
            do iy = g+1, Bs(2)+g
                ...
                do ix = g+1, Bs(1)+g
                    ...
                    ! distance to borders of domain
                    tmp = minval( (/x,-(x-params_nspp%domain_size(1)),-(y-params_nspp%domain_size(2)),-(z-params_nspp%domain_size(3))/) )
    ...
    elseif (params_nspp%sponge_type == "inlet-outlet-x") then
        ! outlet sponge in x-direction
        do ix = g+1, Bs(1)+g
            x = dble(ix-(g+1)) * dx(1) + x0(1)

            ! distance to borders of domain
            tmp = minval( (/x,-(x-params_nspp%domain_size(1))/) )

            sponge(ix,:,:) = step_cosine( tmp, 0.5_rk*params_nspp%L_sponge, 0.5_rk*params_nspp%L_sponge)
        end do
    ...
    elseif (params_nspp%sponge_type == "p-norm") then
        ...
        if ( maxval(abs(params_nspp%domain_size-params_nspp%domain_size(1))) > 1.0e-10_rk) then
            call abort(1610184,"ERROR: for the p-norm sponge, the domain has to be same size in all directions.")
        endif
            ...
            do iz = g+1, Bs(3)+g
                ...
                do iy = g+1, Bs(2)+g
                    ...
                    do ix = g+1, Bs(1)+g
                        ...
                        sponge(ix,iy,iz) = step_cosine( tmp, 0.5_rk*params_nspp%L_sponge, &
                        0.5_rk*params_nspp%L_sponge)
                    end do
                end do
            end do
    ...
    elseif (params_nspp%sponge_type == "p-norm-insect-centered") then
        ! p-norm sponge. The shape of the sponge is dictated as the p-norm
        ! https://de.wikipedia.org/wiki/P-Norm
        ! which is a nice and simple way to get a rectangle with round corners.
