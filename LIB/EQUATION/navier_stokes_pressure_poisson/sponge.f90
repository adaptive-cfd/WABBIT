...
subroutine sponge_2D(sponge, x0, dx, Bs, g)
    ...
    if (.not. params_nspp%initialized) write(*,*) "WARNING: sponge_2D called but ACM not initialized"

    ! cropped domains can differ from 0 and domain_size, so we compute the actual global min/max here
    global_min = params_nspp%domain_cropping_min(1:2) * params_nspp%domain_size(1:2)
    global_max = params_nspp%domain_cropping_max(1:2) * params_nspp%domain_size(1:2)
...
subroutine sponge_3D(sponge, x0, dx, Bs, g)
    ...
    if (.not. params_nspp%initialized) write(*,*) "WARNING: sponge_3D called but ACM not initialized"

    ! cropped domains can differ from 0 and domain_size, so we compute the actual global min/max here
    global_min = params_nspp%domain_cropping_min * params_nspp%domain_size
    global_max = params_nspp%domain_cropping_max * params_nspp%domain_size
