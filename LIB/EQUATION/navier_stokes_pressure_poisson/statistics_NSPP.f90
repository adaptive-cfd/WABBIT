...
subroutine STATISTICS_NSPP( time, dt, u, g, x0, dx, stage, work, mask )
    ...
    case ("post_stage")
        ...
        params_nspp%mean_flow = params_nspp%mean_flow / product(params_nspp%domainSizeCropped(1:dim))
        ...
        if (params_nspp%use_channel_forcing) then
            ...
            V_channel = (params_nspp%domain_size(2)-2.0_rk*params_nspp%h_channel)*params_nspp%domainSizeCropped(1)*params_nspp%domainSizeCropped(3)
            params_nspp%meanflow_channel = params_nspp%meanflow_channel / V_channel

            if ( maxval( abs(params_nspp%domain_size - params_nspp%domainSizeCropped) ) > 1.0e-10_rk ) then
        ...
        if (params_nspp%mpirank == 0) then
            ...
            if (params_nspp%penalization .or. params_nspp%use_sponge) then
                ...
                call append_t_file( 'moments.t', (/time, sum(params_nspp%moment_color(1,1:params_nspp%n_geometries)), &
                sum(params_nspp%moment_color(2,1:params_nspp%n_geometries)), sum(params_nspp%moment_color(3,1:params_nspp%n_geometries)) /) )
                ...
                if (is_insect) then
                    ...
                    do i_insect = 1, n_insects
                        buffer_write(1, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%force_color(:,insects(i_insect)%color_geometry)
                        buffer_write(2, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%moment_color(:,insects(i_insect)%color_geometry)
                    enddo
                    ...
                    ! body
                    do i_insect = 1, n_insects
                        buffer_write(1, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%force_color(:,insects(i_insect)%color_body)
                        buffer_write(2, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%moment_color(:,insects(i_insect)%color_body)
                    enddo
                    ...
                    ! left wing
                    do i_insect = 1, n_insects
                        buffer_write(1, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%force_color(:,insects(i_insect)%color_l)
                        buffer_write(2, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%moment_color(:,insects(i_insect)%color_l)
                    enddo
                    ...
                    ! right wing
                    do i_insect = 1, n_insects
                        buffer_write(1, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%force_color(:,insects(i_insect)%color_r)
                        buffer_write(2, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%moment_color(:,insects(i_insect)%color_r)
                    enddo
                    ...
                    if (twoWingPairs) then
                        ...
                        do i_insect = 1, n_insects
                            if (Insects(i_insect)%second_wing_pair) then
                                buffer_write(1, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%force_color(:,insects(i_insect)%color_l2)
                                buffer_write(2, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%moment_color(:,insects(i_insect)%color_l2)
                            else
                        ...
                        do i_insect = 1, n_insects
                            if (Insects(i_insect)%second_wing_pair) then
                                buffer_write(1, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%force_color(:,insects(i_insect)%color_r2)
                                buffer_write(2, (i_insect-1)*3+1:(i_insect-1)*3+3) = params_nspp%moment_color(:,insects(i_insect)%color_r2)
                            else
                                buffer_write(1, (i_insect-1)*3+1:(i_insect-1)*3+3) = 0.0_rk
                                buffer_write(2, (i_insect-1)*3+1:(i_insect-1)*3+3) = 0.0_rk
                            endif
            ...
            if (params_nspp%nu*params_nspp%enstrophy > 0.0_rk .and. params_nspp%HIT_linear_forcing) then
                ...
                dissipation = params_nspp%dissipation / product(params_nspp%domainSizeCropped(1:dim))

                u_RMS = sqrt( (2.0_rk/3.0_rk) * params_nspp%e_kin / product(params_nspp%domainSizeCropped(1:dim)) )
                
                call append_t_file( 'turbulent_statistics.t', (/time, dissipation, params_nspp%e_kin/product(params_nspp%domainSizeCropped(1:dim)), u_RMS, &
