...
subroutine initial_block_distribution_3D( params, lgt_block, hvy_block, phi )
...
! variables
    ...
    integer(kind=ik)                    :: max_blocks, num_blocks, max_treelevel
    integer(kind=ik)                    :: num_blocks_x, num_blocks_y, num_blocks_z
...
! variables initialization
    ...
    allocate( coord_z( Ds ), stat=allocate_error )
    !call check_allocation(allocate_error)
    if ( allocate_error /= 0 ) then
        write(*,'(80("_"))')
        write(*,*) "ERROR: memory allocation fails"
...
! main body
    ...
    if (rank==0) then
        if ( params%unit_test .eqv. .false. ) then
            write(*,'(80("_"))')
            write(*,'("INIT: Field with res: ", i5, " x", i5, " x", i5, " gives: ", i5, " x", i5, " x", i5, " (", i5, ") blocks of size: ", i5)') Ds, Ds, Ds, num_blocks_x, num_blocks_y, num_blocks_z, num_blocks, Bs
        end if
    end if
    ...
    do i = 1, Ds
        ...
        coord_y(i) = (i-1) * Ly / (Ds-1)
        coord_z(i) = (i-1) * Lz / (Ds-1)
    end do
            ...
            do l = 1, num_blocks_z
                ...
                if (rank == (k-1)) then
                    call new_block_heavy(params, &
                                         ...
                                         phi( (j-1)*(Bs-1) + 1 : j*(Bs-1) + 1 , (i-1)*(Bs-1) + 1 : i*(Bs-1) + 1, (l-1)*(Bs-1) + 1 : l*(Bs-1) + 1 ), &
