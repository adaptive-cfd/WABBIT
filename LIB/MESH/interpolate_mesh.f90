...
subroutine interpolate_mesh()
    ...
    integer(kind=ik)                                :: dF, k, N, block_num, id1, id2, id3, allocate_error, Bs, g, i, free_id
    integer(kind=ik), dimension(10)                 :: me, s1, s2, s3, new_treecode
    integer(kind=ik), dimension(4)                  :: block_ids

    real(kind=rk), dimension(:,:), allocatable      :: new_data, new_data_w_ghost, data_predict_fine, data_predict_coarse
    ...
    do dF = 1, blocks_params%number_data_fields
        ...
        if (blocks(k)%active) then
            ...
            if (blocks(block_num)%refinement == -2) then
                ...
                new_data(1:(Bs-1)/2+1, 1:(Bs-1)/2+1)            = blocks( block_ids(1) )%data_fields(dF)%data_(g+1:Bs+g:2, g+1:Bs+g:2)
                new_data(1:(Bs-1)/2+1, (Bs-1)/2+1:Bs)           = blocks( block_ids(2) )%data_fields(dF)%data_(g+1:Bs+g:2, g+1:Bs+g:2)
                new_data((Bs-1)/2+1:Bs, 1:(Bs-1)/2+1)           = blocks( block_ids(3) )%data_fields(dF)%data_(g+1:Bs+g:2, g+1:Bs+g:2)
                new_data((Bs-1)/2+1:Bs, (Bs-1)/2+1:Bs)          = blocks( block_ids(4) )%data_fields(dF)%data_(g+1:Bs+g:2, g+1:Bs+g:2)
                ...
                call new_block(block_num, new_treecode, 10, new_data_w_ghost, new_coord_x, new_coord_y, Bs, g, dF)
            ...
            elseif (blocks(block_num)%refinement == 1) then
                ...
                data_predict_coarse                             = blocks(block_num)%data_fields(dF)%data_(g+1:Bs+g, g+1:Bs+g)
                ...
                call new_block(free_id, new_treecode, 10, new_data_w_ghost, new_coord_x, new_coord_y, Bs, g, dF)
                ...
                ! write new block on free id = old block
                call new_block(free_id, new_treecode, 10, new_data_w_ghost, new_coord_x, new_coord_y, Bs, g, dF)
