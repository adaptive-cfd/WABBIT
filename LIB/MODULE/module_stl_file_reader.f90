...
module module_stl_file_reader
    ...
    pointTriangleDistance3, compute_superstl_file
...
contains
    ...
    subroutine compute_superstl_file( triangles, facenormals, ntri, superstl )
        ...
        do i = istart, iend
            ...
            if (mpirank == mpisize-1) then
                info = int(100.0*real(i) / real(npercpu))
    ...
    function vertexnormal( x, points, facenormals )
        ...
        do i = 1, ntri
            if (is_same_vector(x, points(i,1:3))) then
                ! faces containing the 1st vertex as 1st point
                N1 = N1 + 1
                faces1(N1) = i
            endif

            if (is_same_vector(x, points(i,4:6))) then
                ! faces containing the 1st vertex as 2nd point
                N2 = N2 + 1
                faces2(N2) = i
            endif

            if (is_same_vector(x, points(i,7:9))) then
                ! faces containing the 1st vertex as 3rd point
                N3 = N3 + 1
                faces3(N3) = i
            endif
        enddo
        ...
        do k = 1, N3
            ...
            vertexnormal = vertexnormal + alpha * unit_vector(facenormals(faces3(k),1:3))
        enddo
    ...
    real(kind=rk) function pointTriangleDistance3(tri1,tri2,tri3,point,normal, n1,n2,n3, e1,e2,e3)
        ! calculate distance between a point and a triangle in 3D
        ! SYNTAX
        !   dist = pointTriangleDistance(TRI,P)
        !   [dist,PP0] = pointTriangleDistance(TRI,P)
        ...
        !  reg4  | reg5    \ reg6
        implicit none
        real(kind=4), dimension(1:3), intent(in) :: tri1,tri2,tri3,normal, n1,n2,n3,e1,e2,e3
        real(kind=rk), dimension(1:3), intent(in) :: point
        real(kind=rk), dimension(1:3) :: BB,EE0,EE1,DD
        real(kind=rk) :: a,b,c,d,e,f,det,s,t,sqrDistance,tmp0,tmp1,numer,denom,invDet
        integer :: region
        ...
        if (t >= 0.d0) then
            pointTriangleDistance3 = dsqrt(sqrDistance)
        else
            pointTriangleDistance3 = -dsqrt(sqrDistance)
        endif
