...
!> \author msr
!
!> \brief put data to RMA window on other proc \n
...
! main body
    ...
    call MPI_Win_create( int_receive_buffer(1,1), int_length, ik, MPI_INFO_NULL, WABBIT_COMM, int_win, ierr )

    ! real window
    real_length = size(real_receive_buffer,1) * size(real_receive_buffer,2) * rk
    call MPI_Win_create( real_receive_buffer(1,1), real_length, rk, MPI_INFO_NULL, WABBIT_COMM, real_win, ierr )
