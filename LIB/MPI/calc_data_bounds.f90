subroutine compute_sender_buffer_bounds(params, ijkrecv, ijksend, ijkbuffer, dir, leveldiff, TYPE)
    ...
    integer(kind=ik) :: i1, i2, g, Bs, nDirs, min_size,Nsender
    ...
    do while ( invalid )
        ...
        do i = 1, params%dim
            ...
            if (i1>=g+1 .and. i1<=Bs+g .and. i2>=g+1 .and. i2<=Bs+g) then
                invalid = .false.
            else
                invalid = .true.
            endif
...
subroutine get_block_spacing_origin( params, treecode, x0, dx )
    ...
    dx = 2.0_rk**(-level) * (/1.0_rk, 1.0_rk, 1.0_rk/) / real(bs-1, kind=rk)
