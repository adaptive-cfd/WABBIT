...
subroutine create_com_list(com_list, com_list_i)
    ...
    integer(kind=ik), dimension(200, 7), intent(out)    :: com_list
        ...
        if ( blocks(light_id)%active == .true. ) then
            ...
            if ( blocks_params%size_domain == blocks_params%size_block ) then
                ...
                do l = 1, 8
                    ...
                    com_list(com_list_i, 7)  = find_neighborhood(light_id, blocks(light_id)%neighbor_dir(l))
                    com_list_i               = com_list_i + 1
                end do

            else
                ! more than one block
                ! loop over all directions, first neighbors
                do l = 1, 8
                    ! neighbor light id
                    ...
                    com_list(com_list_i, 6)  = l
                    com_list(com_list_i, 7)  = find_neighborhood(light_id, blocks(neighbor_light_id)%neighbor_dir(l))
                    com_list_i               = com_list_i + 1
                end do

                ! to do: more than one neighbor
