subroutine restrict_predict_data( params, res_pre_data, ijk, neighborhood, &
    lvl_diff, hvy_block, num_eqn, hvy_id, ignore_Filter, filter_ID )
    ...
    character(len=*), intent(in), optional :: filter_ID
    character(len=cshort) :: filter_ID2
...
! #endif
    ...
    elseif ( lvl_diff == +1) then
        ...
        call restrict_data( params, res_pre_data, ijk, hvy_block, num_eqn, hvy_id, ignore_Filter, filter_ID2 )
...
subroutine restrict_data( params, res_data, ijk, hvy_block, num_eqn, hvy_id, ignore_Filter, filter_ID )
    implicit none
    ...
    if (.not. ignore_Filter) then
        ...
        if (hvy_ID /= restricted_hvy_ID) then
            call restrict_copy_at_CE(params, hvy_block, hvy_ID, nc, filter_ID)
...
subroutine restrict_copy_at_CE(params, hvy_data, hvy_ID, num_eqn, filter_ID, ijk)
    ...
    if (filter_ID == "MGR") then
        HD_l = lbound(params%MGR, dim=1)
        HD_r = ubound(params%MGR, dim=1)
        ! if (.not. present(ijk)) then
            ! filter whole block and don't care about neighbors first
            call blockFilterXYZ_vct( params, hvy_data(:,:,:,1:num_eqn, hvy_id), hvy_restricted(:,:,:,1:num_eqn), params%MGR, HD_l, HD_r, do_restriction=.true.)
        ! else
        ...
        !             hvy_restricted(ijk(1,1)+HD_l:ijk(2,1)+HD_r, ijk(1,2)+HD_l:ijk(2,2)+HD_r, ijk(1,3):ijk(2,3), 1:num_eqn), params%MGR, HD_l, HD_r, do_restriction=.true.)
        !     else
        !         call blockFilterXYZ_wherepossible_vct( params, &
        !             hvy_data(      ijk(1,1)+HD_l:ijk(2,1)+HD_r, ijk(1,2)+HD_l:ijk(2,2)+HD_r, ijk(1,3)+HD_l:ijk(2,3)+HD_r, 1:num_eqn, hvy_id), &
        !             hvy_restricted(ijk(1,1)+HD_l:ijk(2,1)+HD_r, ijk(1,2)+HD_l:ijk(2,2)+HD_r, ijk(1,3)+HD_l:ijk(2,3)+HD_r, 1:num_eqn), params%MGR, HD_l, HD_r, do_restriction=.true.)
        !     endif
        !     ! call abort(197, "Not finished yet but also how did you end up here?")
        ! endif
    elseif (filter_ID == "HD") then
        ...
        call abort(202603141, "FilterID unknown?!")
    endif
