subroutine componentWiseNorm_tree(params, hvy_block, tree_ID, which_norm, norm, norm_case, n_val)
    ...
    integer(kind=ik) :: k, hvy_id, n_eqn, Bs(1:3), g(1:3), p, mpierr, lgt_id, D, level_me, ref_me, norm_case_id
    logical          :: SC_only  
    ...
    case ("Linfty")
        ...
        do k = 1, hvy_n(tree_ID)
            ...
            do p = 1, n_eqn
                norm(p) = max(norm(p), abs(maxval( hvy_block(g(1)+1:Bs(1)+g(1):mod(norm_case_id,10), g(2)+1:Bs(2)+g(2):mod(norm_case_id,10), g(3)+1:Bs(3)+g(3):mod(norm_case_id,10), p, hvy_id ))) )
            enddo
        enddo
