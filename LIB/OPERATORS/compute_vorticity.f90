...
subroutine compute_vorticity(u, v, w, dx, Bs, g, discretization, vorticity)
    ...
    if (size(u,3)>2) then ! 3D case
            ...
            do iz = g+1, Bs(3)+g
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ...
                        v_dx = (a_FD6(-3)*v(ix-3,iy,iz) &
                              + a_FD6(-2)*v(ix-2,iy,iz) &
                              + a_FD6(-1)*v(ix-1,iy,iz) &
                              + a_FD6(+1)*v(ix+1,iy,iz) &
                              + a_FD6(+2)*v(ix+2,iy,iz) &
                              + a_FD6(+3)*v(ix+3,iy,iz))*dx_inv

                        v_dz = (a_FD6(-3)*v(ix,iy,iz-3) &
                              + a_FD6(-2)*v(ix,iy,iz-2) &
                              + a_FD6(-1)*v(ix,iy,iz-1) &
                              + a_FD6(+1)*v(ix,iy,iz+1) &
                              + a_FD6(+2)*v(ix,iy,iz+2) &
                              + a_FD6(+3)*v(ix,iy,iz+3))*dz_inv

                        w_dx = (a_FD6(-3)*w(ix-3,iy,iz) &
                              + a_FD6(-2)*w(ix-2,iy,iz) &
                              + a_FD6(-1)*w(ix-1,iy,iz) &
                              + a_FD6(+1)*w(ix+1,iy,iz) &
                              + a_FD6(+2)*w(ix+2,iy,iz) &
                              + a_FD6(+3)*w(ix+3,iy,iz))*dx_inv

                        w_dy = (a_FD6(-3)*w(ix,iy-3,iz) &
                              + a_FD6(-2)*w(ix,iy-2,iz) &
                              + a_FD6(-1)*w(ix,iy-1,iz) &
                              + a_FD6(+1)*w(ix,iy+1,iz) &
                              + a_FD6(+2)*w(ix,iy+2,iz) &
                              + a_FD6(+3)*w(ix,iy+3,iz))*dy_inv
            ...
            do iz = g+1, Bs(3)+g
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ...
                        v_dx = (a_TW4(-3)*v(ix-3,iy,iz) &
                              + a_TW4(-2)*v(ix-2,iy,iz) &
                              + a_TW4(-1)*v(ix-1,iy,iz) &
                              + a_TW4(+1)*v(ix+1,iy,iz) &
                              + a_TW4(+2)*v(ix+2,iy,iz) &
                              + a_TW4(+3)*v(ix+3,iy,iz))*dx_inv

                        v_dz = (a_TW4(-3)*v(ix,iy,iz-3) &
                              + a_TW4(-2)*v(ix,iy,iz-2) &
                              + a_TW4(-1)*v(ix,iy,iz-1) &
                              + a_TW4(+1)*v(ix,iy,iz+1) &
                              + a_TW4(+2)*v(ix,iy,iz+2) &
                              + a_TW4(+3)*v(ix,iy,iz+3))*dz_inv

                        w_dx = (a_TW4(-3)*w(ix-3,iy,iz) &
                              + a_TW4(-2)*w(ix-2,iy,iz) &
                              + a_TW4(-1)*w(ix-1,iy,iz) &
                              + a_TW4(+1)*w(ix+1,iy,iz) &
                              + a_TW4(+2)*w(ix+2,iy,iz) &
                              + a_TW4(+3)*w(ix+3,iy,iz))*dx_inv

                        w_dy = (a_TW4(-3)*w(ix,iy-3,iz) &
                              + a_TW4(-2)*w(ix,iy-2,iz) &
                              + a_TW4(-1)*w(ix,iy-1,iz) &
                              + a_TW4(+1)*w(ix,iy+1,iz) &
                              + a_TW4(+2)*w(ix,iy+2,iz) &
                              + a_TW4(+3)*w(ix,iy+3,iz))*dy_inv
        ...
        case("FD_4th_central")
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    u_dy = (a_FD4(-2)*u(ix,iy-2,1) &
                          + a_FD4(-1)*u(ix,iy-1,1) &
                          + a_FD4(+1)*u(ix,iy+1,1) &
                          + a_FD4(+2)*u(ix,iy+2,1))*dy_inv

                    v_dx = (a_FD4(-2)*v(ix-2,iy,1) &
                          + a_FD4(-1)*v(ix-1,iy,1) &
                          + a_FD4(+1)*v(ix+1,iy,1) &
        ...
        case("FD_6th_central")
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    u_dy = (a_FD6(-3)*u(ix,iy-3,1) &
                          + a_FD6(-2)*u(ix,iy-2,1) &
                          + a_FD6(-1)*u(ix,iy-1,1) &
                          + a_FD6(+1)*u(ix,iy+1,1) &
                          + a_FD6(+2)*u(ix,iy+2,1) &
                          + a_FD6(+3)*u(ix,iy+3,1))*dy_inv

                    v_dx = (a_FD6(-3)*v(ix-3,iy,1) &
                          + a_FD6(-2)*v(ix-2,iy,1) &
                          + a_FD6(-1)*v(ix-1,iy,1) &
                          + a_FD6(+1)*v(ix+1,iy,1) &
                          + a_FD6(+2)*v(ix+2,iy,1) &
                          + a_FD6(+3)*v(ix+3,iy,1))*dx_inv
        ...
        case("FD_4th_central_optimized")
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    u_dy = (a_TW4(-3)*u(ix,iy-3,1) &
                          + a_TW4(-2)*u(ix,iy-2,1) &
                          + a_TW4(-1)*u(ix,iy-1,1) &
                          + a_TW4(+1)*u(ix,iy+1,1) &
                          + a_TW4(+2)*u(ix,iy+2,1) &
                          + a_TW4(+3)*u(ix,iy+3,1))*dy_inv

                    v_dx = (a_TW4(-3)*v(ix-3,iy,1) &
                          + a_TW4(-2)*v(ix-2,iy,1) &
                          + a_TW4(-1)*v(ix-1,iy,1) &
                          + a_TW4(+1)*v(ix+1,iy,1) &
                          + a_TW4(+2)*v(ix+2,iy,1) &
                          + a_TW4(+3)*v(ix+3,iy,1))*dx_inv
        ...
        case("FD_6th_central")
            do ix = g+1, Bs(1)+g
                do iy = g+1, Bs(2)+g
                    do iz = g+1, Bs(3)+g
                        ! Note: a(0) does NOT appear (it is zero...)
                        u_dy = (a_FD6(-3)*u(ix,iy-3,iz) + a_FD6(-2)*u(ix,iy-2,iz) + a_FD6(-1)*u(ix,iy-1,iz) + a_FD6(+1)*u(ix,iy+1,iz) &
                              + a_FD6(+2)*u(ix,iy+2,iz) + a_FD6(+3)*u(ix,iy+3,iz))*dy_inv

                        u_dz = (a_FD6(-3)*u(ix,iy,iz-3) + a_FD6(-2)*u(ix,iy,iz-2) + a_FD6(-1)*u(ix,iy,iz-1) + a_FD6(+1)*u(ix,iy,iz+1) &
                              + a_FD6(+2)*u(ix,iy,iz+2) + a_FD6(+3)*u(ix,iy,iz+3))*dz_inv

                        v_dx = (a_FD6(-3)*v(ix-3,iy,iz) + a_FD6(-2)*v(ix-2,iy,iz) + a_FD6(-1)*v(ix-1,iy,iz) + a_FD6(+1)*v(ix+1,iy,iz) &
                              + a_FD6(+2)*v(ix+2,iy,iz) + a_FD6(+3)*v(ix+3,iy,iz))*dx_inv

                        v_dz = (a_FD6(-3)*v(ix,iy,iz-3) + a_FD6(-2)*v(ix,iy,iz-2) + a_FD6(-1)*v(ix,iy,iz-1) + a_FD6(+1)*v(ix,iy,iz+1) &
                              + a_FD6(+2)*v(ix,iy,iz+2) + a_FD6(+3)*v(ix,iy,iz+3))*dz_inv

                        w_dx = (a_FD6(-3)*w(ix-3,iy,iz) + a_FD6(-2)*w(ix-2,iy,iz) + a_FD6(-1)*w(ix-1,iy,iz) + a_FD6(+1)*w(ix+1,iy,iz) &
                              + a_FD6(+2)*w(ix+2,iy,iz) + a_FD6(+3)*w(ix+3,iy,iz))*dx_inv

                        w_dy = (a_FD6(-3)*w(ix,iy-3,iz) + a_FD6(-2)*w(ix,iy-2,iz) + a_FD6(-1)*w(ix,iy-1,iz) + a_FD6(+1)*w(ix,iy+1,iz) &
                              + a_FD6(+2)*w(ix,iy+2,iz) + a_FD6(+3)*w(ix,iy+3,iz))*dy_inv
                        vor_abs(ix,iy,iz) = sqrt( (w_dy - v_dz)**2 + (u_dz - w_dx)**2 + (v_dx - u_dy)**2)
                    end do
                end do
            end do
