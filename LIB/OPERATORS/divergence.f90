    ...
    if (size(u,3)>2) then
        ...
        case("FD_6th_central")
            do iz = g+1, Bs(3)+g
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        u_dx = (a_FD6(-3)*u(ix-3,iy,iz) +a_FD6(-2)*u(ix-2,iy,iz) +a_FD6(-1)*u(ix-1,iy,iz) &
                               +a_FD6(+1)*u(ix+1,iy,iz) +a_FD6(+2)*u(ix+2,iy,iz) +a_FD6(+3)*u(ix+3,iy,iz))*dx_inv
                        v_dy = (a_FD6(-3)*v(ix,iy-3,iz) +a_FD6(-2)*v(ix,iy-2,iz) +a_FD6(-1)*v(ix,iy-1,iz) &
                               +a_FD6(+1)*v(ix,iy+1,iz) +a_FD6(+2)*v(ix,iy+2,iz) +a_FD6(+3)*v(ix,iy+3,iz))*dy_inv
                        w_dz = (a_FD6(-3)*w(ix,iy,iz-3) +a_FD6(-2)*w(ix,iy,iz-2) +a_FD6(-1)*w(ix,iy,iz-1) &
                               +a_FD6(+1)*w(ix,iy,iz+1) +a_FD6(+2)*w(ix,iy,iz+2) +a_FD6(+3)*w(ix,iy,iz+3))*dz_inv
            ...
            do iz = g+1, Bs(3)+g
                do iy = g+1, Bs(2)+g
                    do ix = g+1, Bs(1)+g
                        ...
                        v_dy = (a_TW4(-3)*v(ix,iy-3,iz) &
                              + a_TW4(-2)*v(ix,iy-2,iz) &
                              + a_TW4(-1)*v(ix,iy-1,iz) &
                              + a_TW4(+1)*v(ix,iy+1,iz) &
                              + a_TW4(+2)*v(ix,iy+2,iz) &
                              + a_TW4(+3)*v(ix,iy+3,iz))*dy_inv

                        w_dz = (a_TW4(-3)*w(ix,iy,iz-3) &
                              + a_TW4(-2)*w(ix,iy,iz-2) &
                              + a_TW4(-1)*w(ix,iy,iz-1) &
                              + a_TW4(+1)*w(ix,iy,iz+1) &
                              + a_TW4(+2)*w(ix,iy,iz+2) &
                              + a_TW4(+3)*w(ix,iy,iz+3))*dz_inv
        ...
        case("FD_6th_central")
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    u_dx = (a_FD6(-3)*u(ix-3,iy,1) &
                          + a_FD6(-2)*u(ix-2,iy,1) &
                          + a_FD6(-1)*u(ix-1,iy,1) &
                          + a_FD6(+1)*u(ix+1,iy,1) &
                          + a_FD6(+2)*u(ix+2,iy,1) &
                          + a_FD6(+3)*u(ix+3,iy,1))*dx_inv

                    v_dy = (a_FD6(-3)*v(ix,iy-3,1) &
                          + a_FD6(-2)*v(ix,iy-2,1) &
                          + a_FD6(-1)*v(ix,iy-1,1) &
                          + a_FD6(+1)*v(ix,iy+1,1) &
                          + a_FD6(+2)*v(ix,iy+2,1) &
                          + a_FD6(+3)*v(ix,iy+3,1))*dy_inv
        ...
        case("FD_4th_central_optimized")
            do iy = g+1, Bs(2)+g
                do ix = g+1, Bs(1)+g
                    u_dx = (a_TW4(-3)*u(ix-3,iy,1) &
                          + a_TW4(-2)*u(ix-2,iy,1) &
                          + a_TW4(-1)*u(ix-1,iy,1) &
                          + a_TW4(+1)*u(ix+1,iy,1) &
                          + a_TW4(+2)*u(ix+2,iy,1) &
                          + a_TW4(+3)*u(ix+3,iy,1))*dx_inv

                    v_dy = (a_TW4(-3)*v(ix,iy-3,1) &
                          + a_TW4(-2)*v(ix,iy-2,1) &
                          + a_TW4(-1)*v(ix,iy-1,1) &
                          + a_TW4(+1)*v(ix,iy+1,1) &
                          + a_TW4(+2)*v(ix,iy+2,1) &
                          + a_TW4(+3)*v(ix,iy+3,1))*dy_inv
                    div(ix,iy,1) = u_dx + v_dy
                end do
            end do
        case default
