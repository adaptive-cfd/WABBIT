...
subroutine wavelet_test_equi(params)
    use module_globals
    use module_mesh
    use module_params
    use module_mpi
    ...
    allocate(params%symmetry_vector_component(1:params%n_eqn))
    params%symmetry_vector_component(1) = "0"
    params%number_blocks = 2**(params%dim*params%Jmax)
    time = 0.0_rk
    ...
    call saveHDF5_tree("equi-original_222.h5", time, iteration, 1, params, hvy_block, tree_ID )
    ...
    ! modify WC
    !----------------------------------------------------------------------------
    ! select any block and delete its wavelet coefficients
    k = hvy_active(7,tree_ID)
    hvy_block( (g+2):(Bs(1)+g):2, (g+1):(Bs(1)+g):2, :, :, hvy_active(k,tree_ID)) = 0.0_rk
    hvy_block( (g+1):(Bs(1)+g):2, (g+2):(Bs(1)+g):2, :, :, hvy_active(k,tree_ID)) = 0.0_rk
    hvy_block( (g+2):(Bs(1)+g):2, (g+2):(Bs(1)+g):2, :, :, hvy_active(k,tree_ID)) = 0.0_rk
    ...
    do k = 1, hvy_n(tree_ID)
        hvyID = hvy_active(k,tree_ID)
        hvy_tmp(:,:,:,2,hvyID) = hvy_block(:,:,:,1,hvyID) - hvy_tmp(:,:,:,1,hvyID)
    end do

    call saveHDF5_tree("equi-delta_222.h5", time, iteration, 2, params, hvy_tmp, tree_ID )


    call saveHDF5_tree("equi-data_222.h5", time, iteration, 1, params, hvy_block, tree_ID )
    ...
    do k = 1, hvy_n(tree_ID)
        hvyID = hvy_active(k,tree_ID)
        write(*,*) "Block", hvyID, "IWT(FWT(u)) error=", sqrt( sum( (hvy_block(g+1:Bs(1)+g,g+1:Bs(2)+g,:,1,hvyID) - hvy_tmp(g+1:Bs(1)+g,g+1:Bs(2)+g,:,1,hvyID))**2 ) )
    end do
...
! open(unit=32, file="sc.csv", status="replace")
! do ii = 1, Bs(2)+2*g
!     write(32,'(48(es12.4,";"))') sc(:,ii,1,1)
! enddo
! close(32)
! open(unit=32, file="wcx.csv", status="replace")
! do ii = 1, Bs(2)+2*g
!     write(32,'(48(es12.4,";"))') wcx(:,ii,1,1)
! enddo
! close(32)
! open(unit=32, file="wcy.csv", status="replace")
! do ii = 1, Bs(2)+2*g
!     write(32,'(48(es12.4,";"))') wcy(:,ii,1,1)
! enddo
! close(32)
! open(unit=32, file="wcxy.csv", status="replace")
! do ii = 1, Bs(2)+2*g
!     write(32,'(48(es12.4,";"))') wcxy(:,ii,1,1)
! enddo
! close(32)
! stop
