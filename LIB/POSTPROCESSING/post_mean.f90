subroutine post_mean(params)
    ...
    do k = 1, hvy_n(tree_ID)
        ...
        if (params%dim == 3) then
            meanl = meanl + sum( hvy_block(g+1:Bs(1)+g, g+1:Bs(2)+g, g+1:Bs(3)+g, 1, hvy_id))*dx(1)*dx(2)*dx(3)
        else
            meanl = meanl + sum( hvy_block(g+1:Bs(1)+g, g+1:Bs(2)+g, 1, 1, hvy_id))*dx(1)*dx(2)
        endif
    end do
