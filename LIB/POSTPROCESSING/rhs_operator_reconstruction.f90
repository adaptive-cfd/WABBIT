subroutine rhs_operator_reconstruction(params)
    ...
    integer(kind=ik) :: ic, ic2, icc, icc2, ixx, iyy, ix2, iy2, nx_fine, ny_fine, ixx2,iyy2, n_nonzero, nc
    ...
    character(len=80) :: def_wavelet, def_operator, def_filter
    logical :: def_adapt
    ...
    call get_cmd_arg( "--filter", params%filter_type, default= def_filter )! if not set we take the parameter from the ini file
    CFL_number = params%CFL
    call read_ini_file_mpi(ini_file, fname_ini, .true.)
    if (params%physics_type == "ConvectionDiffusion") then
        call read_param_mpi(ini_file, 'ConvectionDiffusion', 'nu', nu, 0.0_rk)
    elseif (params%physics_type == "ACM-new") then
        call read_param_mpi(ini_file, 'ACM-new', 'nu', nu, 0.0_rk)
    else
        nu = 0.0_rk
    endif
    ...
    open(17, file=trim(adjustl(file(:len_trim(file)-3)))//'_info.txt', status='replace')
    ...
    ny_fine = nint(domain(1)/dx_fine)

    write(*,*) "nx_fine=", nx_fine, "ny_fine=", ny_fine, "n_ghost=", g
    write(*,*) "nblocks=", lgt_n, "bs=", Bs, "npoints (op. matrix size!)=", lgt_n*bs(1)*bs(2)

    !---------------------------------------------------------------------------
    ! save the grid (for plotting in python)
    !---------------------------------------------------------------------------
    open(19, file=trim(adjustl(file(:len_trim(file)-3)))//'_grid.txt', status='replace')
    ...
    !---------------------------------------------------------------------------
    open(17, file=trim(adjustl(file(:len_trim(file)-3)))//'_op_mat.txt', status='replace')
    ...
    do iblock = 1, hvy_n(tree_ID_u)
        write(*,'(A, I0, A, I0)') "Block ", iblock, "out of", hvy_n(tree_ID_u)
        do ic = 1, nc
            write(*,'(A, I0, A, I0)') "Component ", ic, "out of", nc
            do ix = g+1, Bs(1)+g
                do iy = g+1, Bs(2)+g
                    !write(*,*) "--------------------point---------------------------"
                    !---------------------------------------------------------------
                    ...
                    ! set the one
                    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    hvy_block(ix, iy, iz, ic, hvy_active(iblock,tree_ID_ei)) = hvy_block(ix, iy, iz, ic, hvy_active(iblock,tree_ID_ei)) + 1.0_rk
                    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    ...
                    if (params%filter_type /= "no_filter") then
                        call sync_ghosts_tree(params, hvy_work(:,:,:,:,:,1), tree_ID_ei)
                        call filter_wrapper(time, params, hvy_work(:,:,:,:,:,1), tree_ID_ei)
                        call sync_ghosts_tree(params, hvy_work(:,:,:,:,:,1), tree_ID_ei)
                    endif
                        ...
                        do ic2 = 1, nc
                            do iy2 = g+1, Bs(2)+g
                                do ix2 = g+1, Bs(1)+g
                                    ...
                                    val = hvy_work(ix2, iy2, iz, ic2, hvy_active(k,tree_ID_rhs_u_ei),1) ! u_dx

                                    if (abs(val) > 1.0e-13) then
                                        ! this point is a nonzero value
                                        write(17,'(i6,1x,i6,1x,es15.8)') ixx+(iyy-1)*nx_fine+(ic-1)*ny_fine*nx_fine, ixx2+(iyy2-1)*nx_fine+(ic2-1)*ny_fine*nx_fine, val
                                        ! write(*,*) "python col=", ixx2+(iyy2-1)*nx_fine -1 , "row=", ixx+(iyy-1)*nx_fine -1, val, "xy=",x,y, "block", k
                                    endif
                                    hvy_work(ix2, iy2, iz, ic2, hvy_active(k,tree_ID_rhs_u_ei),1) = 0
                                enddo
                            enddo
                        enddo
                    enddo
