...
!
    ...
    allocate( block_old(Bs+2*g, Bs+2*g, Bs+2*g), r(Bs+2*g, Bs+2*g, Bs+2*g), sigma(Bs+2*g, Bs+2*g, Bs+2*g) )
    ...
    if ( params%threeD_case ) then
        ...
        do i = g-1, Bs+g+2
            do j = g-1, Bs+g+2
                do l = g-1, Bs+g+2
                    ...
                    ! filter
                    block_data(i, j, l ) = block_data(i, j, l ) + phi_tilde(1) + phi_tilde(2) + phi_tilde(3)
        ...
        do i = g-1, Bs+g+2
            do j = g-1, Bs+g+2
                ...
                ! filter
                block_data(i, j, 1) = block_data(i, j, 1 ) + phi_tilde(1) + phi_tilde(2)
    ...
    if ( params%threeD_case ) then
        ...
        do i = g, Bs+g+1
            do j = g, Bs+g+1
                do l = g, Bs+g+1
                    ...
                    ! x direction
                    call shock_detector_1D( block_data(i-1:i+1, j, l), block_old(i, j, l), r_xyz(1) )
                    ! y direction
                    call shock_detector_1D( block_data(i, j-1:j+1, l), block_old(i, j, l), r_xyz(2) )
                    ! z direction
                    call shock_detector_1D( block_data(i, j, l-1:l+1), block_old(i, j, l), r_xyz(3) )
                    ! max r
                    r(i, j, l) = max( r_xyz(1), r_xyz(2), r_xyz(3) )
                    ! sigma
        ...
        do i = g, Bs+g+1
            do j = g, Bs+g+1
                    ...
                    ! x direction
                    call shock_detector_1D( block_data(i-1:i+1, j, 1), block_old(i, j, 1), r_xyz(1) )
                    ! y direction
                    call shock_detector_1D( block_data(i, j-1:j+1, 1), block_old(i, j, 1), r_xyz(2))
                    ! max r
                    r(i, j, 1) = max( r_xyz(1), r_xyz(2) )
                    ! sigma
            ...
            do j = g+1, Bs+g
                do l = g+1, Bs+g
                    ...
                    phi_tilde(1) = 0.5_rk*( sigma(i+1, j, l) + sigma(i, j, l) ) &
                                   * ( block_old(i-1, j, l ) * stencil(1) + block_old(i, j, l ) * stencil(2)/2.0_rk ) &
                                   + 0.5_rk*( sigma(i-1, j, l) + sigma(i, j, l) ) &
                                   * ( block_old(i, j, l ) * stencil(2)/2.0_rk + block_old(i+1, j, l ) * stencil(3))
                    ! y direction
                    phi_tilde(2) = 0.5_rk*( sigma(i, j+1, l) + sigma(i, j, l) ) &
                                   * ( block_old(i, j-1, l ) * stencil(1) + block_old(i, j, l ) * stencil(2)/2.0_rk ) &
                                   + 0.5_rk*( sigma(i, j-1, l) + sigma(i, j, l) ) &
                                   * ( block_old(i, j, l ) * stencil(2)/2.0_rk + block_old(i, j+1, l ) * stencil(3))

                    ! z direction
                    phi_tilde(3) = 0.5_rk*( sigma(i, j, l+1) + sigma(i, j, l) ) &
                                   * ( block_old(i, j, l -1) * stencil(1) + block_old(i, j, l ) * stencil(2)/2.0_rk ) &
                                   + 0.5_rk*( sigma(i, j, l-1) + sigma(i, j, l) ) &
                                   * ( block_old(i, j, l ) * stencil(2)/2.0_rk + block_old(i, j, l+1 ) * stencil(3))


                    ! filter
                    block_data(i, j, l ) = block_old(i, j, l ) + phi_tilde(1) + phi_tilde(2) + phi_tilde(3)
            ...
            do j = g+1, Bs+g
                ...
                ! x direction
                    phi_tilde(1) = 0.5_rk*( sigma(i+1, j, 1) + sigma(i, j, 1) ) &
                                   * ( block_old(i-1, j, 1 ) * stencil(1) + block_old(i, j, 1 ) * stencil(2)/2.0_rk ) &
                                   + 0.5_rk*( sigma(i-1, j, 1) + sigma(i, j, 1) ) &
                                   * ( block_old(i, j, 1 ) * stencil(2)/2.0_rk + block_old(i+1, j, 1 ) * stencil(3))
                    ! y direction
                    phi_tilde(2) = 0.5_rk*( sigma(i, j+1, 1) + sigma(i, j, 1) ) &
                                   * ( block_old(i, j-1, 1 ) * stencil(1) + block_old(i, j, 1 ) * stencil(2)/2.0_rk ) &
                                   + 0.5_rk*( sigma(i, j-1, 1) + sigma(i, j, 1) ) &
                                   * ( block_old(i, j, 1 ) * stencil(2)/2.0_rk + block_old(i, j+1, 1 ) * stencil(3))

                    ! filter
                    block_data(i, j, 1 ) = block_old(i, j, 1 ) + phi_tilde(1) + phi_tilde(2)
...
subroutine shock_detector_1D( phi, phi_old, r )
    ...
    r = 0.5_rk * ( ( phi(2) - phi(3) )**2 + ( phi(2) - phi(1) )**2 ) / ( phi_old + 1.0e-16_rk )**2 + 1.0e-16_rk
