                ...
                do k = 1, hvy_n
                    ...
                    do j = 2, size(rk_coeffs, 2) 
                        if ( abs(rk_coeffs(size(rk_coeffs, 1),j)) < 1e-8_rk) then
                            ...
                            ! ... dt*(b1*k1 + b2*k2+ ..)
                            ! rk_coeffs(size(rk_coeffs,1)) , since rk_coeffs we want to access last line,  e.g. b1 = butcher(last line,2)
                            hvy_block( :, :, :, dF, hvy_active(k)) = hvy_block( :, :, :, dF, hvy_active(k)) + dt*rk_coeffs(size(rk_coeffs,1),j) * hvy_work( :, :, :, (dF-1)*5+j, hvy_active(k))
                        end if
                    end do
                end do
            end do
                ...
                do k = 1, hvy_n
                    ...
                    do j = 2, size(rk_coeffs, 2) 
                        if ( abs(rk_coeffs(size(rk_coeffs, 1),j)) < 1e-8_rk) then
                            ...
                            hvy_block( :, :, :, 1:N_dF, hvy_active(k)) = hvy_block( :, :, :, 1:N_dF, hvy_active(k)) + dt*rk_coeffs(size(rk_coeffs,1),j) * hvy_work( :, :, :, (j-1)*N_dF+1:j*N_dF, hvy_active(k))
                        end if
                    end do
                end do
