                ...
                if (abs(rk_coeffs(l)) < 1e-8_rk) then
                    ...
                    do dF = 1, N_dF
                        ...
                        do k = 1, hvy_n
                            ...
                            hvy_block( :, :, :, dF, hvy_active(k)) = hvy_block( :, :, :, dF, hvy_active(k)) + dt * rk_coeffs(l) * hvy_work( :, :, :, (dF-1)*5+l, hvy_active(k))
                        end do
                    end do
                end if
            end do
            ...
            do l = 2, j
                ...
                if (abs(rk_coeffs(l)) < 1e-8_rk) then
                    ...
                    do k = 1, hvy_n
                        ! new input for computation of k-coefficients
                        ! k_j = RHS(data_field(t) + sum(a_jl*k_l))
                        hvy_block( :, :, :, 1:N_dF, hvy_active(k)) = hvy_block( :, :, :, 1:N_dF, hvy_active(k)) + dt * rk_coeffs(l) * hvy_work( :, :, :, (l-1)*N_dF+1:l*N_dF, hvy_active(k))
                    end do
                end if
            end do
