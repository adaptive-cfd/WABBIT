...
! --------------------------------
!
! time step main function, RK2
!
! name: time_step_RK2.f90
! date: 28.09.2016
! author: msr
! version: 0.1
!
...
subroutine time_step_RK2(time)
    ...
    if ( blocks_params%number_data_fields == 1 ) then
        ...
        do k = 1, N
            ...
            block_num                                   = blocks_params%active_list(k)
            blocks(block_num)%data_fields(dF)%data_     = blocks(block_num)%data_fields(dF)%data_old + (2.0_rk/3.0_rk) * dt * blocks(block_num)%data_fields(dF)%k1
        ...
        do k = 1, N
            ...
            blocks(block_num)%data_fields(dF)%data_     = blocks(block_num)%data_fields(dF)%data_old + dt/4.0_rk * ( blocks(block_num)%data_fields(dF)%k1 &
                                                                            + 3.0_rk*blocks(block_num)%data_fields(dF)%k2 )
